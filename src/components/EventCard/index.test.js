...
describe("When a event card is created", () => {
  it("an image is display with alt value", () => {
    ...
    // Vérifie que l'élément de l'image est dans le document.
    expect(imageElement).toBeInTheDocument();
    expect(imageElement.alt).toEqual("image-alt-text");
  });
  it("a title, a label, and a month are displayed", () => {
    // Rend le composant EventCard avec des propriétés spécifiques.
    render(
      <EventCard
    ...
    const monthElement = screen.getByText(/avril/i);
