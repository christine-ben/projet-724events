...
const Page = () => {
  ...
  ****************** */
  ...
  return <>
    ...
    <main>
    {/* une section de diaporama (Slider), */}
      <section className="SliderContainer">
        <Slider />
      </section>
      {/* correction ajout des id */} 
      ...
      {/* La section d'événements  */}
      <section className="EventsContainer">
        <h2 className="Title" id="nos-realisations">Nos réalisations</h2>
        <EventList />
      </section>
      {/* La section de l'équipe (PeoplesContainer) affiche des cartes de personnes (PeopleCard) 
      avec des images, des noms et des positions. */}
    ...
    {/* La section du  (footer)  */}
    <footer className="row">
      <div className="col presta">
        <h3>Notre derniére prestation</h3>
        {/* correction on ajoute "last" pour supprimer les erreurs "undefined" de la console 
            => Avec l'opérateur "&&", on restitue conditionnellement le composant****** 
            L'expression last?.cover && agit comme une condition  si last?.cover est vrai 
            (propriété cover non "undefined"), alors le composant EventCard sera rendu. */}
        {last?.cover && (
    <EventCard
      imageSrc={last?.cover}
      title={last?.title}
